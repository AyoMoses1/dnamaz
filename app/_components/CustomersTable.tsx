...
const CustomersTable: FC = () => {
  ...
  const selectedAccountType = searchParams?.get("userType") as "Individual" | "Cooperate" || "Individual";
  ...
  const debouncedSearch = useCallback(
    debounce((value: string) => {
      router.push(`?${createQueryString({ search: value, page: "1" })}`, { scroll: false });
    }, 500),
    [router, searchParams]
  );
  ...
  const { data, isLoading, isError } = useQuery<CustomerResponse>({
    queryKey: ["customers", { userType: selectedAccountType, search: searchTerm, page, pageSize }],
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`?${createQueryString({ page: String(newPage) })}`, { scroll: false });
  };
  ...
  if (isLoading) return (
    <>
      <Spinner />
      <div className="text-primary-200 text-xl mt-5">Loading...</div>
    </>
  );
  ...
  if (isError) return (
    <>
      <Spinner />
      <div className="text-red-600 text-xl mt-5 border">Error fetching customers...</div>
    </>
  );
              ...
              {customers.length > 0 ? (
                customers.map((customer) => (
                  <tr key={customer.userId} className="border-b">
                    ...
                    <td className="px-4 py-4">
                      <div>
                        <p className="font-medium text-sm text-gray-500">{customer.occupation || "N/A"}</p>
